...
public abstract class CharacterEncoder
{
	...
	/**
	 * 109 * Encode the suffix that ends every output line. By default 110 * this
	 * method just prints a <newline> into the output stream. 111
	 */
	 ...
	 * 137 * to the output stream. This method will run until it exhausts the 138
	 * * input stream, but does not print the line suffix for a final 139 * line
	 * that is shorter than bytesPerLine(). 140
	 */
	...
	/**
	 * 202 * Return a byte array from the remaining bytes in this ByteBuffer. 203
	 * *
	 * <P>
	 ...
	 * 207 * byte array backing the ByteBuffer. If this is not possible, a 208 *
	 * new byte array will be created. 209
	 */
	private byte[] getBytes(ByteBuffer bb)
	{
		...
		/*
		 * 218 * If it has a usable backing byte buffer, use it. Use only 219 * if
		 * the array exactly represents the current ByteBuffer. 220
		 */
		if (bb.hasArray())
		{
			byte[] tmp = bb.array();
		...
		{
			/*
			 * 232 * This class doesn't have a concept of encode(buf, len, off), 233 *
			 * so if we have a partial buffer, we must reallocate 234 * space. 235
			 */
	 ...
	 * <P>
	 * 251 * The ByteBuffer's position will be advanced to ByteBuffer's limit. 252
	 */
	 ...
	 * <P>
	 * 263 * The ByteBuffer's position will be advanced to ByteBuffer's limit. 264
	 */
	 ...
	 * 272 * to the output stream. This method will run until it exhausts the 273
	 * * input stream. It differs from encode in that it will add the 274 * line
	 * at the end of a final line that is shorter than bytesPerLine(). 275
	 */
	 ...
	 * <P>
	 * 335 * The ByteBuffer's position will be advanced to ByteBuffer's limit. 336
	 */
	 ...
	 * <P>
	 * 347 * The ByteBuffer's position will be advanced to ByteBuffer's limit. 348
	 */
